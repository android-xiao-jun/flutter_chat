...
//            new Handler().postDelayed(this::inputTitleDialog, 1000);//显示输入框
            new Handler().postDelayed(this::showNextScreen, 1000);//显示输入框
        } else {
    ...
    private void inputTitleDialog() {
        ...
        SharedPreferences sp_ip=getSharedPreferences("config_ip",MODE_PRIVATE);

        String ip_host = sp_ip.getString("ip_host","10.0.0.85");

        inputServer.setText(ip_host);
        ...
        builder.setPositiveButton("确定", (dialog, which) -> {
            ...
            sp_ip.edit().putString("ip_host",inputName).commit();
            //正常走后面
            showNextScreen();
        });
        builder.show();
